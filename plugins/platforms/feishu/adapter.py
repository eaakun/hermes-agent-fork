...
class FeishuAdapterSettings:
    ...
    card_streaming: bool = True  # main-reply Card Kit v2 streaming (feishu.card_streaming)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            card_streaming=os.getenv("FEISHU_CARD_STREAMING", "true").strip().lower() != "false",
        )
    ...
    def _apply_settings(self, settings: FeishuAdapterSettings) -> None:
        ...
        # Bot avatar (from /bot/v3/info) + cached card-header icon img_key.
        # The header icon of a Card Kit v2 card needs an uploaded image key
        # (custom_icon), so we fetch the avatar bytes once and cache the key.
        self._bot_avatar_url: str = ""
        self._card_header_icon_img_key: str = ""
        ...
        self._card_streaming = settings.card_streaming
    ...
    async def send(
        ...
        # Main reply card streaming: the gateway's stream consumer signals a
        # preview that it will keep editing by passing expect_edits in the
        # stream-created metadata (see _metadata_for_send in stream_consumer).
        # Render that preview as a Card Kit v2 card and edit it in place via
        # edit_message, instead of sending a stream of plain text messages.
        if self._card_streaming and (metadata or {}).get("expect_edits"):
            return await self._send_streaming_card_reply(
                chat_id=chat_id,
                content=content,
                reply_to=reply_to,
                metadata=metadata,
    ...
    async def _send_streaming_card_reply(
        self,
        chat_id: str,
        content: str,
        *,
        ...
        """Send the main reply as a Card Kit v2 streaming card.

        The streaming card carries the reply body as a single markdown element
        so the gateway's edit_message() can update it in place on each frame.
        Fall back to a plain send when the card POST is rejected (e.g. the
        card body exceeds the platform's card size limit).

        The stream consumer maps a turn-final first send to ``notify`` in the
        stream-created metadata (alongside ``expect_edits``), so a single-shot
        final delivery renders as a completed card instead of a stuck
        in-progress one (no follow-up finalize edit happens in that path).
        """
        try:
            _is_final_first = bool((metadata or {}).get("notify"))
            # Best-effort: ensure the DM avatar is uploaded for the header icon
            # (cached after the first call; never blocks the reply on failure).
            try:
                await self._upload_card_header_icon()
            except Exception:
                pass
            response = await _send_card_v2(
                self,
                chat_id,
                self._build_streaming_reply_card(content, finalize=_is_final_first),
                reply_to=reply_to,
                metadata=metadata,
            )
            result = self._finalize_send_result(response, "card streaming send failed")
        except Exception as exc:
            result = None
            logger.warning(
                "[Feishu] Streaming card preview send raised; falling back to plain text: %s", exc
            )
        if result is None or not result.success:
            # Strip the expect_edits marker so the fallback does not re-enter
            # the streaming-card path below (infinite recursion guard).
            fallback_meta = dict(metadata or {})
            fallback_meta.pop("expect_edits", None)
            return await self.send(
                chat_id=chat_id,
                content=content,
                reply_to=reply_to,
                metadata=fallback_meta or None,
            )
        if result.message_id:
            _streaming_cards[str(result.message_id)] = self._build_streaming_reply_card(
                content, finalize=_is_final_first
            )
        return result
    ...
    async def send_card(
        ...
        *,
        ...
        """Send a Card Kit v2 interactive card with optional reply_to.

        Args:
            chat_id: Feishu chat ID.
            card: Pre-built card dict (from build_card, card_div, etc.).
            reply_to: Optional message_id to reply to (enables reply-based streaming).
        ...
        try:
            ...
            result = self._finalize_send_result(response, "send_card failed")
            return result
        except Exception as exc:
            logger.error("[Feishu] send_card error: %s", exc, exc_info=True)
            return SendResult(success=False, error=str(exc))

    async def render_progress_card(
        self,
        chat_id: str,
        text: str,
        progress_lines: list,
        ...
        """Render a Card Kit v2 progress card for tool execution updates.

        Args:
            chat_id: Feishu chat ID.
            text: Progress text content.
            progress_lines: List of progress lines accumulated so far.
            reply_to: Optional message_id to reply to.
            metadata: Optional metadata dict.

        Returns:
            SendResult if card was sent, None to fall back to plain text.
        """
        if not self._card_builders_available:
            return None
        try:
            tool_badge = card_tool_badge(
                tool_name=progress_lines[0].split()[1] if len(progress_lines) > 0 and progress_lines[0][1:].split() else "tool",
                status="running",
            )
            progress_elem = card_progress(percentage=50)
            note = card_note(f"Running: {text[:60]}...")
            body_elems = [
                card_div(elements=[tool_badge]),
                progress_elem,
                note,
            ]
            card = build_card(
                header={"title": {"content": "Hermes Working", "tag": "plain_text"}, "template": "blue"},
                body=body_elems,
            ...
            logger.error("[Feishu] render_progress_card error: %s", exc, exc_info=True)
            return None

    @property
    def _card_builders_available(self) -> bool:
        """True when Card Kit v2 builders (card_div, card_columns, etc.) are importable."""
        return True

    def _build_streaming_reply_card(self, content: str, *, finalize: bool = False) -> dict:
        """Build the streaming main-reply Card Kit v2 card.

        Single-element body so each in-place edit updates the full reply text.
        ``finalize`` marks the last frame (green header + done footer).
        The header shows the DM avatar (cached ``img_key`` from the bot avatar,
        uploaded by ``_upload_card_header_icon``) plus the bot's name.
        """
        body = [
            {"tag": "markdown", "content": content or "…"},
            card_hr(),
            card_note("✅ 生成完成" if finalize else "⏳ 生成中…"),
        ]
        header: dict = {
            "title": self._bot_name or "1cook",
            "template": "green" if finalize else "blue",
        }
        if self._card_header_icon_img_key:
            header["icon"] = {"tag": "custom_icon", "img_key": self._card_header_icon_img_key}
        return build_card(
            header=header,
            body=body,
            config={"wide_screen_mode": True, "update_multi": True},
    ...
    async def edit_message(
        ...
        *,
        ...
        # Streaming main reply: this message_id is a live Card Kit v2 card
        # (registered by _send_streaming_card_reply / send_streaming_card).
        # Update the card in place so the user sees a single rich reply sweep
        # through its full text instead of back-to-back edits.
        if self._card_streaming and str(message_id) in _streaming_cards:
            result = await update_streaming_card(
                self,
                str(message_id),
                self._build_streaming_reply_card(content, finalize=finalize),
            )
            if result.success and finalize:
                # Final frame — the stream is over; drop the registry entry so
                # the in-memory card state does not grow without bound across
                # conversations.
                _streaming_cards.pop(str(message_id), None)
            return result
        try:
    ...
    async def _maybe_extract_text_document(self, cached_path: str, media_type: str) -> str:
        ...
        try:
            ...
            if ext in {".txt", ".md"} and media_type in {"text/plain", "text/markdown", ""}:
            ...
            if ext == ".pdf" or media_type == "application/pdf":
                ...
                doc = _fitz.open(cached_path)
                parts = []
                for page in doc:
                    parts.append(page.get_text())
                doc.close()
                content = "\n".join(parts)
                if len(content) > _MAX_TEXT_INJECT_BYTES:
                    content = content[:_MAX_TEXT_INJECT_BYTES] + "\n... [truncated]"
                display_name = self._display_name_from_cached_path(cached_path)
                return f"[Content of {display_name}]:\n{content}"
            return ""
    ...
    async def _hydrate_bot_identity(self) -> None:
        ...
        try:
            ...
            if content:
                ...
                avatar_url = (parsed.get("bot_avatar_url") or "").strip()
                if open_id:
                ...
                if avatar_url:
                    self._bot_avatar_url = avatar_url
        except Exception:
            logger.debug(
    ...
    async def _upload_card_header_icon(self) -> str:
        """Upload the bot avatar and cache its image key for card headers.

        Card Kit v2 header icons must use ``custom_icon`` with an ``img_key``
        obtained via the image-upload API — an external URL is not accepted.
        Fetches the bot's avatar bytes (from ``_bot_avatar_url``, populated by
        ``_hydrate_bot_identity``) once, uploads them, and caches the key so a
        streaming reply card can render the DM avatar as its header icon.
        Best-effort: returns "" (no icon) on any failure.
        """
        if self._card_header_icon_img_key:
            return self._card_header_icon_img_key
        if not self._client or not self._bot_avatar_url:
            return ""
        try:
            with urlopen(self._bot_avatar_url, timeout=10) as resp:
            ...
            image_key = self._extract_response_field(upload_response, "image_key")
            if not image_key:
                logger.warning("[Feishu] Card header avatar upload missing image_key")
                return ""
            self._card_header_icon_img_key = str(image_key)
            logger.info("[Feishu] Card header icon cached: %s", image_key)
            return self._card_header_icon_img_key
        except Exception as exc:
            logger.warning("[Feishu] Failed to upload card header avatar: %s", exc)
            return ""
...
def _apply_yaml_config(yaml_cfg: dict, feishu_cfg: dict) -> dict | None:
    ...
    # Card Kit v2 reply streaming: when the feishu config enables it,
    # progressive main-reply content is delivered as a rich card instead
    # of plain text.  Bridged to env like allow_bots (config.yaml-driven).
    if "card_streaming" in feishu_cfg and not os.getenv("FEISHU_CARD_STREAMING"):
        os.environ["FEISHU_CARD_STREAMING"] = str(feishu_cfg["card_streaming"]).lower()
    return None
...
# =============================================================================
# Card Kit v2 element builders
# =============================================================================

def card_columns(*children: dict, width: str = "weighted", weight: int = 1) -> dict:
    """Build a ``column_set`` with equal-weighted columns (default 2).

    Args:
        width: ``weighted`` (default) or ``adaptive``.
        weight: relative width weight per column (default 1).
    """
    return {
        "tag": "column_set",
        "width": width,
        "columns": [
            {**{"tag": "column", "width": width, "weight": weight}, **child}
            for child in children
        ],
    }


def card_div(text: str = None, *, elements: list[dict] = None, **kwargs) -> dict:
    """Build a ``div`` element compatible with schema v2.

    Uses ``fields`` + ``text`` (Feishu v2 canonical form) for plain_text/lark_md.
    For plain_text: {'tag': 'div', 'fields': [{'text': {'tag': 'plain_text', 'content': ...}, 'is_short': False}]}
    For lark_md:   {'tag': 'div', 'fields': [{'text': {'tag': 'lark_md', 'content': ...}, 'is_short': False}]}

    Args:
        text: text content (auto-selects plain_text or lark_md based on **kwargs).
        elements: explicit list of Feishu element dicts (raw elements form).
    """
    if elements is not None:
        # Raw elements form (legacy / non-field layout)
        result: dict = {"tag": "div", "elements": elements}
    elif text is not None:
        tag = "lark_md" if any(kwargs.get(k) for k in ("bold", "italic", "language")) else "plain_text"
        if kwargs.get("bold"):
            content = f"**{text}**"
        elif kwargs.get("italic"):
            content = f"_{text}_"
        elif kwargs.get("language"):
            content = f"```{kwargs['language']}\n{text}\n```"
        else:
            content = text
        result = {
            "tag": "div",
            "fields": [{"text": {"tag": tag, "content": content}, "is_short": False}],
        }
    else:
        result = {"tag": "div", "elements": []}
    return result


def card_note(text: str, max_chars: int = 500) -> dict:
    """Build a note-like element for schema v2 (v2 dropped ``note`` tag).

    Schema v2 does not support ``note`` or ``background_color`` on div.
    Renders as a plain ``div`` with a prefix marker for visual distinction.
    Auto-truncated to ``max_chars``.
    """
    ...
    return {
        ...
        "fields": [
            {
                "text": {"tag": "lark_md", "content": f"[NOTE] {text}"},
                "is_short": False,
            }
        ],
    }
...
def card_progress(
    ...
    *,
    ...
    Schema v2 does not support the ``progress`` tag; returns a
    ``markdown`` element that renders a text-based progress bar.
    percentage is 0-100. text overrides the default label.
    """
    pct = max(0, min(100, percentage))
    filled = round(pct / 10)
    empty = 10 - filled
    bar = "█" * filled + "░" * empty
    label = text if text else f"{pct}%"
    return {"tag": "markdown", "content": f"`{bar}` {label}"}


def card_collapsible(title: str, body_elements: list[dict], collapsed: bool = False) -> dict:
    """Build a collapsible section.

    Schema v2 does not support ``collapsible_panel``; returns a
    ``markdown`` element that renders the title with a toggle marker.
    The body_elements are ignored (no true collapse in v2) — include
    them as a follow-up element after this in body.elements.
    """
    icon = "▶" if collapsed else "▼"
    return {"tag": "markdown", "content": f"**{icon} {title}**"}


def card_tool_badge(tool_name: str, status: str = "running", callback_value: str = "") -> dict:
    """Build a button-style status badge for a tool call."""
    status_icons = {"running": "[RUN]", "done": "[OK]", "error": "[ERR]"}
    status_colors = {"running": "default", "done": "primary", "error": "danger"}
    icon = status_icons.get(status, "[RUN]")
    color = status_colors.get(status, "default")
    label = f"{icon} {tool_name}"
    value = {"tool": tool_name, "status": status}
    if callback_value:
        value["cb"] = callback_value
    return {
        "tag": "button",
        "text": {"tag": "plain_text", "content": label},
        "type": color,
...
def card_action(*buttons: dict) -> list[dict]:
    """Build an ``action`` element wrapping one or more buttons.

    Schema v2 does not support the ``action`` tag; returns buttons
    as a flat list so they can be spliced directly into body.elements.
    """
    return list(buttons)


def card_button(label: str, value: str = "", action_name: str = "", btn_type: str = "default") -> dict:
    """Build a generic button element.

    Args:
        label: button label text.
        value: value dict passed back in callback events.
        action_name: ``hermes_action`` key set in the value.
        btn_type: ``default`` | ``primary`` | ``danger``.
    """
    v = value if isinstance(value, dict) else {"action": value}
    if action_name:
        v["hermes_action"] = action_name
    return {
        "tag": "button",
        "text": {"tag": "plain_text", "content": label},
        "type": btn_type,
...
def card_tag(text: str, color: str = "grey") -> dict:
    """Build a ``tag`` element (coloured label chip)."""
    return {
        "tag": "tag",
...
def card_image(url: str, alt: str = "") -> dict:
    """Build an ``img`` element."""
    result: dict = {"tag": "img", "src": url}
    if alt:
        result["alt"] = {"tag": "plain_text", "content": alt}
    return result


def card_hr() -> dict:
    """Build a ``hr`` (horizontal rule) divider element."""
    return {"tag": "hr"}
...
def build_card(
    *,
    header: dict = None,
    body: list[dict] = None,
    footer: list[dict] = None,
    config: dict = None,
) -> dict:
    """Build a Card Kit v2 interactive card payload.

    Args:
        header:  dict with ``title`` (str) and optional ``template`` (str)
                 and ``icon`` (Card Kit v2 header icon dict, e.g.
                 ``{"tag": "custom_icon", "img_key": "img_v2_..."}``).
        body:    list of element dicts (markdown str auto-wrapped).
        footer:  list of element dicts.
        config:  extra config fields merged into ``config`` block.

    Example::

        build_card(
            header={"title": "报告摘要", "template": "blue"},
            body=[
                {"tag": "markdown", "content": "**进度**: 60%"},
                card_columns(
                    card_div("完成数: 12"),
                    card_div("进行中: 3"),
                ),
                card_note("数据更新于 2026-08-17"),
            ],
        )
    """
    card: dict = {
        ...
        "config": {"wide_screen_mode": True, "update_multi": True},
        "header": {
            "title": {"tag": "plain_text", "content": header.get("title", "") if header else ""},
            "template": header.get("template", "blue") if header else "blue",
        },
        "body": {"elements": []},
    }
    if header and header.get("icon"):
        card["header"]["icon"] = header["icon"]
    if config:
        card["config"].update(config)
    if body:
        for item in body:
            if isinstance(item, str):
                item = {"tag": "markdown", "content": item}
            card["body"]["elements"].append(item)
    if footer:
        card["footer"] = {"elements": footer}
    return card
...
async def _send_card_v2(
    ...
    *,
    reply_to: str = None,
    update_message_id: str = None,
    metadata: dict = None,
) -> SendResult:
    """Internal helper: send or update a Card Kit v2 card.

    If ``update_message_id`` is provided, edits that message instead of sending.
    ``reply_to`` is ignored when updating.
    """
    ...
    if update_message_id:
        # Interactive cards cannot be updated via the PUT message.update API
        # (code 230054 "not supported for this message type"). Feishu updates
        # cards in place through the PATCH message endpoint, which takes only
        # the card content (no msg_type) in the request body.
        body = adapter._build_patch_message_body(content=payload)
        request = adapter._build_patch_message_request(
            message_id=update_message_id, request_body=body,
        )
        response = await adapter._run_blocking(adapter._client.im.v1.message.patch, request)
        result = adapter._finalize_send_result(response, "card update failed")
        if result.success:
            result.message_id = update_message_id
        return result
...
# =============================================================================
# Reply-based streaming helpers (for the gateway's progress consumer)
# =============================================================================

_streaming_cards: dict[str, dict] = {}
...
async def send_streaming_card(
    ...
    """Send a Card Kit v2 card and return (result, message_id) for later editing.

    Use ``update_streaming_card(adapter, message_id, card)`` to edit in place.
    """
    result = await _send_card_v2(adapter, chat_id, card, reply_to=reply_to, metadata=metadata)
    msg_id = getattr(result, "message_id", None) or ""
    if msg_id:
        _streaming_cards[msg_id] = card
...
async def update_streaming_card(
    ...
    """Edit an existing streaming card in place (reply-based streaming)."""
    _streaming_cards[message_id] = card
    return await _send_card_v2(adapter, chat_id=None, card=card, update_message_id=message_id)


# Alias the helper so gateway/run.py can call it via getattr
FeishuAdapter._send_card_v2 = staticmethod(_send_card_v2)
FeishuAdapter._streaming_cards = _streaming_cards
FeishuAdapter.send_streaming_card = staticmethod(send_streaming_card)
FeishuAdapter.update_streaming_card = staticmethod(update_streaming_card)
...
def register(ctx) -> None:
    ...
    # Register payload pre-processor for Feishu encryption
    # This handles AES-256-CBC decryption of Feishu webhook payloads
    try:
        from gateway.platforms.webhook import WebhookAdapter
        _register_feishu_encryption_hook()
    except ImportError:
        pass  # Webhook adapter not available


def _register_feishu_encryption_hook() -> None:
    """Register Feishu encryption pre-processor with the webhook adapter.

    This handles AES-256-CBC decryption of Feishu webhook payloads
    that come through the generic webhook adapter.
    """
    import hashlib
    import json
    import os

    async def _feishu_decrypt_pre_processor(payload: dict, request) -> dict:
        """Decrypt Feishu encrypted webhook payloads."""
        if not isinstance(payload, dict) or "encrypt" not in payload:
            return payload

        feishu_key = os.environ.get("FEISHU_ENCRYPT_KEY", "")
        if not feishu_key:
            logger.warning("[Feishu] encrypt key not configured for webhook")
            return payload
        ...
        try:
            ...
            encrypted = b64.b64decode(payload["encrypt"])
            iv = encrypted[: AES.block_size]
            ct = encrypted[AES.block_size:]
            key = hashlib.sha256(feishu_key.encode("utf-8")).digest()
            ...
            if pad_len < 1 or pad_len > AES.block_size:
                pad_len = 0
            pt = pt[:-pad_len] if pad_len else pt
            decrypted_str = pt.decode("utf-8", errors="replace")
            decrypted_payload = json.loads(decrypted_str)
            logger.info(
                "[Feishu] Decrypted webhook payload: keys=%s",
                list(decrypted_payload.keys()) if isinstance(decrypted_payload, dict) else "N/A",
            )
    ...
    # Store the pre-processor for later registration with webhook adapter
    # The webhook adapter will call this during connect()
    _feishu_encryption_pre_processor = _feishu_decrypt_pre_processor
