...
DEFAULT_CONFIG = {
    ...
    "free_pool": {
        # OpenCode Zen keyless free-tier rotation pool. When a cron job (or any
        # opencode-free provider use) runs, only models that currently answer
        # on the relay are used, ranked by recent health and circuit-broken
        # after repeated failures.
        "enabled": True,
        # Candidate slugs to probe. Empty list => the built-in default set
        # (OPENCODE_ZEN_FREE_CANDIDATE_MODELS in hermes_cli/models.py).
        "candidates": [],
        # Per-probe HTTP timeout (seconds).
        "probe_timeout": 8.0,
        # How long a successful probe result is cached (seconds).
        "cache_ttl": 600,
        # Consecutive probe/inference failures before a model is circuit-broken
        # out of the fresh pool.
        "circuit_break_failures": 3,
        # Consecutive fully-empty probes before the outage is escalated to ERROR
        # and (optionally) pushed to Feishu.
        "outage_alert_threshold": 3,
        # Persist per-model health to disk so it survives a gateway restart.
        "persist_health": True,
        "persist_path": "",   # empty => $HERMES_HOME/cron/.free_pool_health.json
        # Background probe ticker (seconds between runs). 0 disables it; the
        # pool is then only refreshed lazily on cron ticks / API calls.
        "background_probe_interval": 120,
        # Push a Feishu message when the relay looks fully down. Requires
        # free_pool.feishu_webhook_url (or env FREE_POOL_FEISHU_WEBHOOK).
        "feishu_alert": False,
        "feishu_webhook_url": "",
    },
    "cron": {
