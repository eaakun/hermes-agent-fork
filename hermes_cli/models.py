...
# --- Backported from v0.20.5: OpenCode Zen keyless free-tier runtime ---
# The free pool is served ANONYMOUSLY on the Zen relay. The relay 401s any
# unknown bearer, but accepts a request whose Authorization carries an EMPTY
# bearer value (``Bearer `` with nothing after it). The OpenAI SDK refuses a
# ``None`` api_key but accepts an empty string, and for an empty key it emits
# exactly ``Authorization: Bearer `` — so an empty api_key is the wire form we
# want. We keep a placeholder constant for reference but do NOT put it on the
# wire (it would 401). See opencode_zen_free_headers().
...
def opencode_zen_free_headers() -> dict:
    ...
    ``Authorization: ""`` is belt-and-suspenders: it documents the anonymous
    contract and, on SDK builds where default_headers win over auth_headers,
    neutralizes the wire header. The authoritative mechanism is the empty
    ``api_key`` in opencode_zen_free_runtime(), which makes the SDK emit
    ``Bearer `` (empty value) — the exact form the Zen relay accepts.
...
def opencode_zen_free_runtime(provider_id, model_id) -> Optional[dict]:
    ...
    Zen relay with the keyless placeholder whenever ``provider_id`` is
    ``opencode-free`` (every model on it routes anonymously — the provider's
    contract). Backported from v0.20.5 so local 0.19.0 can use the free pool
    without a Zen subscription key.
    """
    fam = str(provider_id or "").strip().lower()
    if fam != "opencode-free":
        return None
    normalized = normalize_opencode_model_id("opencode-zen", model_id)
    ...
    return {
        ...
        "api_mode": api_mode,
        "base_url": base_url,
    # The OpenAI SDK drops an empty api_key entirely (no Authorization header
    # on the wire → relay 401s). It also overrides default_headers with its own
    # ``Bearer <api_key>``. So we must send a NON-EMPTY placeholder api_key (to
    # keep the agent's explicit-credentials branch active and satisfy the SDK's
    # non-empty requirement) AND override the wire Authorization to empty via
    # default_headers — the Zen relay accepts an empty Authorization for the
    # anonymous free tier. See opencode_zen_free_headers() and agent_init.py's
    # opencode.ai host branch.
...
# --- Pre-probe for the OpenCode Zen free-tier rotation pool ---
# The free catalog is dynamic: some slugs are stale ("not supported") or have
# an unavailable upstream (503). Rather than hard-coding a fallback list that
# rots, we probe the relay at runtime and only put LIVE models into the
# rotation/fallback pool. Backported-from-v0.20.5 technique + local probe.
# Full candidate set = the free slugs the OpenCode Zen relay actually serves
# (from GET /zen/v1/models) plus the v0.20.5 hard-coded list. The pre-probe
# keeps ONLY the ones that answer at run time, so adding stale/delisted
# slugs here is harmless — they simply never enter the rotation pool.
# `x-preview-f-free` is delisted from the live endpoint but kept for
# back-compat; the probe filters it out.
OPENCODE_ZEN_FREE_CANDIDATE_MODELS = [
    "laguna-s-2.1-free",
    "hy3-free",
...
_FREE_POOL_CACHE: dict = {"models": None, "ts": 0.0, "ttl": 600.0}
_FREE_POOL_LOCK = threading.Lock()

# Per-model health memory for the free pool. We rank the primary by best
# recent track record, and circuit-break models that fail repeatedly — whether
# the lightweight pre-probe or a real inference call reports the failure.
_FREE_MODEL_HEALTH: dict = {}
# Consecutive pre-probe outages before we escalate the log to ERROR and treat
# the relay as likely fully down (see _free_pool_outage_state()).
_FREE_MODEL_CIRCUIT_BREAK_FAILED = 3
_FREE_POOL_OUTAGE_THRESHOLD = 3


def record_free_model_outcome(slug: str, ok: bool) -> None:
    """Feed a real inference result back into free-pool health memory.

    Called by the agent fallback path when an opencode-free model actually
    succeeds or fails a real call (not just the lightweight pre-probe). This is
    what lets a model that *probes* live but *fails on real work* get demoted /
    circuit-broken instead of sitting at the top of the rotation.
    """
    slug = str(slug or "").strip().lower()
    if not slug or "-free" not in slug:
        return
    with _FREE_POOL_LOCK:
        h = _FREE_MODEL_HEALTH.setdefault(slug, {"ok": 0, "fail": 0, "cf": 0, "ts": 0.0})
        if ok:
            h["ok"] += 1
            h["cf"] = 0
        else:
            ...
            h["cf"] += 1
        h["ts"] = time.time()
    _ensure_health_loaded()
    _persist_free_pool_health()


def _free_pool_health_score(slug: str) -> float:
    h = _FREE_MODEL_HEALTH.get(slug)
    if not h:
        return 0.0
    return float(h["ok"] - h["fail"])


def _free_pool_outage_state_path() -> str:
    try:
        from hermes_constants import get_hermes_home
        base = get_hermes_home()
    except Exception:
        base = os.path.expanduser("~/.hermes")
    return os.path.join(str(base), "cron", ".free_pool_outage")


def _record_free_pool_outage() -> bool:
    """Bump the consecutive-outage counter on disk. Returns True once the
    threshold is exceeded (relay likely fully down)."""
    import json
    p = _free_pool_outage_state_path()
    try:
        data = json.loads(open(p).read()) if os.path.exists(p) else {}
    except Exception:
        data = {}
    data["consecutive"] = int(data.get("consecutive", 0)) + 1
    ...
    try:
        ...
        pass
    return data["consecutive"] >= int(_get_free_pool_cfg().get("outage_alert_threshold", 3))


def _reset_free_pool_outage() -> None:
    p = _free_pool_outage_state_path()
    try:
        if os.path.exists(p):
            os.remove(p)
...
# ----- config-driven tuning + persistence + alerting + background ticker -----

_FREE_POOL_CFG_CACHE: dict = {"data": None}


def _get_free_pool_cfg() -> dict:
    """Read free_pool tuning from config.yaml, falling back to built-in defaults.

    Cached process-wide; edit config.yaml and restart the gateway to apply.
    """
    if _FREE_POOL_CFG_CACHE["data"] is not None:
        return _FREE_POOL_CFG_CACHE["data"]
    defaults = {
        "enabled": True,
        "candidates": [],
        "probe_timeout": 8.0,
        ...
        "persist_health": True,
        "persist_path": "",
        "background_probe_interval": 120,
        "feishu_alert": False,
        "feishu_webhook_url": "",
    }
    try:
        from hermes_cli.config import load_config
        raw = (load_config() or {}).get("free_pool") or {}
        if isinstance(raw, dict):
            defaults.update(raw)
    except Exception:
        pass
    try:
        defaults["probe_timeout"] = float(defaults.get("probe_timeout", 8.0))
        defaults["cache_ttl"] = float(defaults.get("cache_ttl", 600))
        defaults["circuit_break_failures"] = int(defaults.get("circuit_break_failures", 3))
        defaults["outage_alert_threshold"] = int(defaults.get("outage_alert_threshold", 3))
        defaults["background_probe_interval"] = int(defaults.get("background_probe_interval", 120))
    except Exception:
        pass
    _FREE_POOL_CFG_CACHE["data"] = defaults
    _ensure_health_loaded()
    return defaults


def _free_pool_candidates() -> list:
    cands = _get_free_pool_cfg().get("candidates") or []
    if cands:
        return [str(c).strip() for c in cands if str(c).strip()]
    return list(OPENCODE_ZEN_FREE_CANDIDATE_MODELS)


def _free_pool_health_path() -> str:
    p = (str(_get_free_pool_cfg().get("persist_path") or "").strip())
    if p:
        return p
    try:
        from hermes_constants import get_hermes_home
        ...
        base = os.path.expanduser("~/.hermes")
    return os.path.join(str(base), "cron", ".free_pool_health.json")


def _load_free_pool_health() -> None:
    if not _get_free_pool_cfg().get("persist_health", True):
        return
    p = _free_pool_health_path()
    try:
        if os.path.exists(p):
            import json
...
_FREE_POOL_HEALTH_LOADED = [False]


def _ensure_health_loaded() -> None:
    if _FREE_POOL_HEALTH_LOADED[0]:
        return
    _FREE_POOL_HEALTH_LOADED[0] = True
    _load_free_pool_health()


_FREE_POOL_PERSIST_LAST = [0.0]


def _persist_free_pool_health() -> None:
    if not _get_free_pool_cfg().get("persist_health", True):
        return
    now = time.time()
    if now - _FREE_POOL_PERSIST_LAST[0] < 10.0:  # throttle disk writes
        return
    _FREE_POOL_PERSIST_LAST[0] = now
    p = _free_pool_health_path()
    try:
        import json
        with _FREE_POOL_LOCK:
            snap = {k: dict(v) for k, v in _FREE_MODEL_HEALTH.items()}
        os.makedirs(os.path.dirname(p), exist_ok=True)
        open(p, "w").write(json.dumps(snap))
    except Exception:
        pass


def _free_pool_feishu_alert(message: str) -> None:
    """Best-effort Feishu push when the relay looks fully down. Two modes:

    * Custom bot webhook — set ``free_pool.feishu_webhook_url`` to a
      ``https://open.feishu.cn/open-apis/bot/v2/hook/...`` address.
    * App mode — set ``free_pool.feishu_chat_id`` to a group open_chat_id and
      ensure ``FEISHU_APP_ID`` / ``FEISHU_APP_SECRET`` are available (e.g. in
      ``.env``); the alert is sent via the Feishu message API to that group.

    No-op when disabled or when no destination is configured.
    """
    try:
        cfg = _get_free_pool_cfg()
        if not cfg.get("feishu_alert", False):
            return
        url = (str(cfg.get("feishu_webhook_url") or "").strip()
               or os.environ.get("FREE_POOL_FEISHU_WEBHOOK", "").strip())
        chat_id = str(cfg.get("feishu_chat_id") or "").strip()
        # Mode 1: custom bot webhook (open.feishu.cn bot hook)
        if url.startswith("https://open.feishu.cn/open-apis/bot/v2/hook/"):
            ...
            return
        # Mode 2: app-mode send to a chat via the Feishu message API
        if chat_id:
            import os as _os, json as _json, httpx as _httpx
            app_id = _os.environ.get("FEISHU_APP_ID") or ""
            app_secret = _os.environ.get("FEISHU_APP_SECRET") or ""
            ...
            token = tok.get("tenant_access_token") or tok.get("access_token")
            if not token:
                return
            _httpx.post(
...
_FREE_POOL_TICKER_STARTED = [False]


def _free_pool_ticker_loop(interval: int) -> None:
    import logging as _logging
    # Route through gateway.run so the lines reach gateway.log (Hermes filters
    # the hermes_cli.* namespace out of the gateway log by design).
    _log = _logging.getLogger("gateway.run")

    # Diagnostic marker: proves the ticker function was actually invoked
    # (catches the case where import-time config gating silently skipped it).
    try:
        import os as _os
        _dir = _os.path.expanduser("~/.hermes/cron")
        _os.makedirs(_dir, exist_ok=True)
        with open(_os.path.join(_dir, ".ticker_called"), "a") as _f:
            _f.write("called=%s\n" % time.time())
    except Exception:
        pass

    # The gateway loads its config through a different path than load_config(),
    # so values read at import time can be stale/empty. We launch unconditionally
    # and re-read the live interval each loop iteration (defaulting to the passed
    # value) so the ticker works regardless of when config becomes available.
    time.sleep(3)  # let the gateway finish early startup / logging setup
    _log.warning("free-pool ticker thread running (interval=%ds)", interval)
    try:
        import os as _os
        _os.makedirs(_os.path.expanduser("~/.hermes/cron"), exist_ok=True)
        open(_os.path.expanduser("~/.hermes/cron/.free_pool_ticker_ran"), "w").write(
            "started=%s" % time.time()
        )
    except Exception:
        pass
    while True:
        iv = interval
        try:
            iv = int(_get_free_pool_cfg().get("background_probe_interval", interval) or interval)
        except Exception:
            pass
        if iv <= 0:
            iv = interval
        try:
            pto = 8.0
            try:
                pto = float(_get_free_pool_cfg().get("probe_timeout", 8.0))
            except Exception:
                pass
            fresh = probe_opencode_free_models(timeout=pto)
            if fresh:
                _reset_free_pool_outage()
            else:
                severe = _record_free_pool_outage()
                if severe:
                    thr = 3
                    try:
                        thr = int(_get_free_pool_cfg().get("outage_alert_threshold", 3))
                    except Exception:
                        pass
                    msg = (
                        "opencode free relay appears FULLY DOWN: background probe "
                        "found no live models for %d consecutive probes." % thr
                    )
                    _log.error(msg)
                    _free_pool_feishu_alert(msg)
            _persist_free_pool_health()
            _log.info(
                "free-pool ticker: probed %d live model(s)",
                len(fresh) if fresh else 0,
            )
        except Exception as _e:
            _log.warning("free-pool ticker probe error: %s", _e)
        time.sleep(iv)


def _start_free_pool_ticker() -> None:
    if _FREE_POOL_TICKER_STARTED[0]:
        return
    _FREE_POOL_TICKER_STARTED[0] = True
    try:
        interval = 120
        try:
            interval = int(_get_free_pool_cfg().get("background_probe_interval", 120) or 120)
        except Exception:
            interval = 120
        if interval <= 0:
            interval = 120
        t = threading.Thread(target=_free_pool_ticker_loop, args=(interval,), daemon=True)
        t.name = "free-pool-ticker"
        t.start()
        import logging as _logging
        _logging.getLogger("gateway.run").warning(
            "free-pool ticker launched (interval=%ds)", interval
        )
    except Exception as _e:
        _FREE_POOL_TICKER_STARTED[0] = False
        try:
            import logging as _logging
            _logging.getLogger(__name__).warning("free-pool ticker launch failed: %s", _e)
        except Exception:
            pass


def probe_opencode_free_models(timeout: float = 8.0) -> list:
    """Return the free-tier slugs that currently answer on the Zen relay.

    Uses the same anonymous contract as opencode_zen_free_runtime: a non-empty
    placeholder api_key (so the SDK emits a header) plus an empty
    ``Authorization`` default header. A model is "live" when it returns a 200
    carrying a ``choices`` array (even an empty one); 401/403/429/500/503 and
    "not supported" all exclude it.
    """
    try:
        from openai import OpenAI
    except Exception:
        return []
    candidates = _free_pool_candidates()

    def _try(m: str):
        try:
            c = OpenAI(
                base_url=_OPENCODE_ZEN_FREE_BASE_URL,
                api_key=OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER,
                default_headers=opencode_zen_free_headers(),
                max_retries=0,
                timeout=timeout,
            ...
            if getattr(r, "choices", None) is not None:
                record_free_model_outcome(m, True)
                return m
        except Exception:
            record_free_model_outcome(m, False)
            return None
        record_free_model_outcome(m, False)
        return None

    out: list = []
    # Probe in parallel so a few dead upstreams don't dominate the wall-clock.
    with ThreadPoolExecutor(max_workers=max(1, len(candidates))) as ex:
        for m in ex.map(_try, candidates):
            if m:
                out.append(m)
    return out


def opencode_zen_free_rotation_pool(
    exclude: Optional[str] = None,
    force: bool = False,
    timeout: float = 8.0,
    ttl: float = 600.0,
) -> list:
    """Cached, probed-working free-model slugs suitable as a fallback pool.

    ``exclude`` drops the job's primary model (already attempted first). The
    probe result is memoized per-process with a TTL so a burst of cron ticks
    shares one probe instead of hammering the relay.

    Stale fallback: if a live probe returns nothing (relay fully down, or
    every candidate temporarily unavailable), we do NOT overwrite the last
    known-good pool with an empty list. We return that cached pool even past
    its TTL, so a job still gets *some* free model to try instead of silently
    falling through to nothing. The very first run with nothing live anywhere
    has no fallback and returns [].

    Health ranking: live models are ordered by recent track record (most
    reliable first) so the scheduler picks the steadiest as primary. Models
    that fail repeatedly (probed or real) are circuit-broken out of fresh
    pools; a stale pool keeps every model but still ranks the healthier first.

    All tuning (enabled, candidates, timeouts, thresholds, persistence,
    background ticker, Feishu alerting) comes from the ``free_pool`` section
    of config.yaml.
    """
    _ensure_health_loaded()
    _start_free_pool_ticker()
    if not _get_free_pool_cfg().get("enabled", True):
        return []
    cfg = _get_free_pool_cfg()
    ttl = float(cfg.get("cache_ttl", ttl))
    cb_limit = int(cfg.get("circuit_break_failures", 3))
    outage_thr = int(cfg.get("outage_alert_threshold", 3))
    now = time.time()
    with _FREE_POOL_LOCK:
        cached = _FREE_POOL_CACHE["models"]
        cached_fresh = cached is not None and (now - _FREE_POOL_CACHE["ts"]) < ttl
        if not force and cached_fresh:
            pool = list(cached)
            stale_used = False
        else:
            fresh = probe_opencode_free_models(timeout=float(cfg.get("probe_timeout", timeout)))
            if fresh:
                _FREE_POOL_CACHE["models"] = list(fresh)
                _FREE_POOL_CACHE["ts"] = now
                _reset_free_pool_outage()
                pool = list(fresh)
                stale_used = False
            else:
                if cached:
                    ...
                    if severe:
                        _logging.getLogger(__name__).error(
                            "opencode free relay appears FULLY DOWN: pre-probe "
                            "found no live models for %d consecutive probes; "
                            "using stale pool (age=%.0fs): %s",
                            outage_thr,
                            now - _FREE_POOL_CACHE["ts"], cached,
                        )
                        _free_pool_feishu_alert(
                            "opencode free relay FULLY DOWN: %d consecutive empty "
                            "probes; using stale pool %s" % (outage_thr, cached)
                        )
                    else:
                        _logging.getLogger(__name__).warning(
                            "opencode free pre-probe found no live models; using "
                            "last successful pool (stale, age=%.0fs): %s",
                            now - _FREE_POOL_CACHE["ts"], cached,
                        )
                    pool = list(cached)
                    stale_used = True
                    ...
                    pool = []
                    stale_used = False
        # Rank by health (most reliable first). Fresh pools also drop
        # circuit-broken models; a stale pool keeps all of them (last resort).
        ranked = sorted(pool, key=_free_pool_health_score, reverse=True)
        if not stale_used:
            ranked = [
                m for m in ranked
                if _FREE_MODEL_HEALTH.get(m, {}).get("cf", 0) < cb_limit
            ]
    excl = {str(exclude or "").strip().lower()}
    return [m for m in ranked if m.lower() not in excl]
...
# Start the free-pool background ticker at import time so the gateway keeps the
# live pool fresh without waiting for the first cron task. Guarded by
# _FREE_POOL_TICKER_STARTED and a no-op when background_probe_interval <= 0.
_start_free_pool_ticker()
