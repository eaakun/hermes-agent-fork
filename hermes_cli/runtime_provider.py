...
def resolve_runtime_provider(
    *,
    ...
    # OpenCode Zen free tier (opencode-free): served ANONYMOUSLY on the Zen
    # relay. Route free slugs through the keyless runtime BEFORE the
    # credential-pool / api_key paths so they work with any credential state,
    # including none. Backported from v0.20.5.
    if requested_provider == "opencode-free":
        from hermes_cli.models import opencode_zen_free_runtime as _oc_free_runtime_fn
        _free_runtime = _oc_free_runtime_fn("opencode-free", target_model)
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if pconfig.requires_api_key and not has_usable_secret(creds.get("api_key")):
