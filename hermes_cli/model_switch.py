...
def list_authenticated_providers(
    ...
    *,
    ...
    # Backported from v0.20.5: the keyless opencode-free provider surfaces the
    # Portal's free-model pool. Seed a curated list so the /model picker shows
    # the provider (it would otherwise be dropped for having zero models).
    # Single source of truth: the same candidate set the scheduler probes,
    # so the /model picker and the live rotation pool never drift apart.
    from hermes_cli.models import OPENCODE_ZEN_FREE_CANDIDATE_MODELS
    curated["opencode-free"] = list(OPENCODE_ZEN_FREE_CANDIDATE_MODELS)
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if getattr(overlay, "keyless", False):
            # Keyless providers (e.g. opencode-free) are served anonymously —
            # no credential to check, so always visible in the /model picker.
            has_creds = True
