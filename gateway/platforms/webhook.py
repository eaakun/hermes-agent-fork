...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Plugin hooks for payload pre-processing (e.g. Feishu encryption)
        self._payload_pre_processors: Dict[str, Any] = {}

    # ------------------------------------------------------------------
    # Plugin hooks for payload pre-processing
    # ------------------------------------------------------------------

    def register_payload_pre_processor(self, route_name: str, processor: Any) -> None:
        """Register a payload pre-processor for a specific route.

        The processor is called after payload parsing but before route-specific
        processing. It can decrypt, transform, or validate the payload.

        Args:
            route_name: The route name to register the processor for.
            processor: An async callable with signature:
                async def processor(payload: dict, request: web.Request) -> dict
                Returns the (possibly modified) payload.
        """
        self._payload_pre_processors[route_name] = processor

    async def _pre_process_payload(
        self, route_name: str, payload: dict, request: "web.Request"
    ) -> dict:
        """Call registered pre-processors for the route.

        Also handles built-in pre-processing for known platforms (e.g. Feishu).
        ...
        """
        # Check for registered pre-processors
        processor = self._payload_pre_processors.get(route_name)
        if processor:
            try:
                payload = await processor(payload, request)
            except Exception as e:
                logger.error(
                    "[webhook] Pre-processor failed for route %s: %s",
                    route_name,
                    e,
                )

        # Built-in: Feishu encrypted payload decryption
        if isinstance(payload, dict) and "encrypt" in payload and route_name == "feishu":
            payload = await self._decrypt_feishu_payload(payload)

        return payload

    async def _decrypt_feishu_payload(self, payload: dict) -> dict:
        """Decrypt Feishu AES-256-CBC encrypted webhook payload.

        This is a built-in pre-processing step for Feishu routes.
        The actual decryption logic is in the Feishu adapter plugin,
        but we call it here for backward compatibility.
        """
        feishu_key = os.environ.get("FEISHU_ENCRYPT_KEY", "")
        if not feishu_key:
            logger.warning("[webhook] Feishu encrypt key not configured")
            return payload
        ...
        try:
            ...
            logger.info(
                "[webhook] Feishu decrypted: keys=%s",
                list(decrypted_payload.keys()) if isinstance(decrypted_payload, dict) else "N/A",
            )
            return decrypted_payload
        except Exception as e:
            logger.error("[webhook] Feishu decrypt failed: %s", e)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        # Backward-compat alias: /webhook/{route_name} (without 's') so
        # external services that registered the singular URL still hit the
        # same handler. Required for Feishu webhooks that were configured
        # when the 1cook Worker was doing path translation.
        app.router.add_post("/webhook/{route_name}", self._handle_webhook)
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        if route_config.get("auth") == "skip":
            pass  # auth handled inside relay handler; skip webhook-level HMAC
        else:
        ...
        # Plugin hook: pre-process payload (e.g. Feishu encryption)
        # Plugins can register a pre_process_payload callback to decrypt
        # or transform payloads before they are processed by the webhook adapter.
        payload = await self._pre_process_payload(route_name, payload, request)

        # ── 1bot-relay: proxy message through Hermes Feishu app ──────
        # 1bot webhook_server calls this endpoint instead of calling Feishu API
        # directly with its own app token (which has no access to user's DM).
        # Skip the normal event-type filter chain; go straight to direct delivery.
        if route_name == "1bot-relay" and route_config.get("deliver_only"):
            import os as _os
            secret = _os.environ.get("GATEWAY_RELAY_SHARED_KEY", "")
            sig = request.headers.get("X-1bot-Signature", "")
            body = await request.read()
            if secret and sig != hmac.new(secret.encode(), body, hashlib.sha256).hexdigest():
                logger.warning("[webhook] 1bot-relay: invalid signature")
            ...
            if not chat_id or not content:
                return web.json_response({"ok": False, "error": "chat_id and content required"}, status=400)
            # deliver_extra carries the chat_id override for the feishu adapter
            delivery = {
                "deliver": "feishu",
            ...
            try:
                ...
                return web.json_response({
                    "ok": result.success,
                    "message_id": getattr(result, "message_id", None) or None,
                    "error": result.error if not result.success else None,
                })
            except Exception as e:
                logger.exception("[webhook] 1bot-relay delivery failed")
        ...
        # Pre-process Feishu payload: extract plain text from message content
        # so the prompt template can use {event.message.content.text}.
        # Feishu content can be: {"text": "plain"} or {"title": "", "content": [[...], [...], [...]]]}
        if route_name == "feishu" and isinstance(payload, dict):
            try:
                event_obj = payload.get("event", {})
                msg = event_obj.get("message", {}) if isinstance(event_obj, dict) else {}

                # Feishu sends two webhook events per message:
                # 1) Notification event: {"event": {"message_id_list": [...], "reader": ...}} — no message content
                # 2) Message event: {"event": {"message": {...}, "sender": ...}} — has actual content
                # Skip notification events to avoid creating garbage sessions with unresolved templates.
                if not msg or not isinstance(msg, dict) or not msg.get("content"):
                    logger.info(
                        "[webhook] feishu: skipping notification event (no message content), event_keys=%s",
                        list(event_obj.keys()) if isinstance(event_obj, dict) else "N/A",
                    )
                    return web.json_response({"status": "ok", "skipped": "notification_event"})
                ...
                if isinstance(content, str):
                    ...
                    if isinstance(parsed_content, dict):
                        ...
                        # Rich text message: {"title": "", "content": [[...], [...], [...]]}
                        elif "content" in parsed_content:
                            lines = []
                            for section in parsed_content.get("content", []):
        ...
        # ── Feishu sender_type filter ─────────────────────────────
        # Filter out events from the bot itself (sender_type=app).
        # Feishu sends bot's own outbound messages back as inbound events.
        feishu_sender_type = None
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(event_obj, dict):
                ...
                if isinstance(msg, dict):
                    feishu_event_id = msg.get("message_id")
            header = payload.get("header")
            if isinstance(header, dict) and not feishu_event_id:
                feishu_event_id = header.get("event_id")
        logger.info(
            "[webhook] Feishu event: sender_type=%s event_id=%s payload_type=%s full_payload=%s",
            feishu_sender_type,
            feishu_event_id,
            type(payload).__name__,
            json.dumps(payload)[:500] if isinstance(payload, dict) else str(payload)[:500],
        )
        # The Feishu echo filter only applies to the Feishu route — other
        # routes (github, svix, generic) have no Feishu sender_type and must
        # not be swallowed. deliver_only routes accept pushes from external
        # services (monitoring, other agents) that carry no Feishu sender, so
        # the filter must not apply to them either.
        if (
            route_name == "feishu"
            and not route_config.get("deliver_only")
            and feishu_sender_type not in ("user", None)
        ):
            logger.info("[webhook] Skipping non-user event (sender_type=%s)", feishu_sender_type)
            return web.json_response({"status": "ignored", "reason": "not_user_message"}, status=200)

        # ── Feishu event_id dedup ─────────────────────────────────
        # Skip duplicate events using header.event_id or event.message.message_id.
        if feishu_event_id:
            event_key = f"feishu_event:{feishu_event_id}"
            if not self._record_delivery_id(event_key, now):
                logger.info(
                    "[webhook] Skipping duplicate Feishu event %s (delivery=%s)",
                    feishu_event_id,
                    delivery_id,
                )
                return web.json_response(
                    {"status": "duplicate", "event_id": feishu_event_id},
                    status=200,
                )
        ...
        # Use Feishu chat_id as session key so same chat continues conversation.
        # Fall back to delivery_id for concurrent webhooks on the same route.
        feishu_chat_id = None
        if isinstance(payload, dict):
            event_obj = payload.get("event")
            if isinstance(event_obj, dict):
                msg = event_obj.get("message")
                if isinstance(msg, dict):
                    feishu_chat_id = msg.get("chat_id")
        if feishu_chat_id:
            session_chat_id = f"webhook:{route_name}:{feishu_chat_id}"
        else:
        ...
        # For Feishu (and other routes that set source.chat_id to the bare
        # platform chat_id), also index by the bare chat_id so webhook.send()
        # can find the delivery config when called with event.source.chat_id.
        if feishu_chat_id:
            self._delivery_info[feishu_chat_id] = deliver_config
            self._delivery_info_created[feishu_chat_id] = now
            self._delivery_info_order.append((now, feishu_chat_id))
        self._prune_delivery_info(now)

        # Build source and event
        # For Feishu routes, use feishu_chat_id directly so the session key
        # is stable across turns (webhook:feishu:... prefix breaks build_session_key).
        # Extract sender open_id from Feishu event for proper user identification.
        feishu_sender_id = None
        if route_name == "feishu" and isinstance(payload, dict):
            event_obj = payload.get("event", {})
            if isinstance(event_obj, dict):
                sender = event_obj.get("sender", {})
                if isinstance(sender, dict):
                    feishu_sender_id = sender.get("id") or sender.get("open_id")
        if route_name == "feishu" and feishu_chat_id:
            source = self.build_source(
                chat_id=feishu_chat_id,
                chat_name=f"webhook/{route_name}",
                chat_type="dm",
                user_id=feishu_sender_id or f"webhook:{route_name}",
    ...
    async def on_processing_complete(
        ...
        Exception: Feishu DMs (chat_id starts with "oc_") are multi-turn
        conversations and must NOT be ended after each message.
        """
        # Skip session ending for Feishu DMs — they are multi-turn and need
        # to preserve history across messages.
        if event.source.chat_id and event.source.chat_id.startswith("oc_"):
            return
    ...
    def _validate_signature(
        ...
        # Feishu: X-Lark-Signature = sha256(timestamp + nonce + encrypt_key + raw_body)
        # Required headers: X-Lark-Request-Timestamp, X-Lark-Request-Nonce
        # Feishu uses the ENCRYPT_KEY (not the HMAC secret) for signing
        lark_sig = _header("X-Lark-Signature")
        if lark_sig:
            lark_ts = _header("X-Lark-Request-Timestamp")
            lark_nonce = _header("X-Lark-Request-Nonce")
            if not lark_ts or not lark_nonce:
                logger.warning("[webhook] Feishu signature headers incomplete")
                return False
            feishu_key = os.environ.get("FEISHU_ENCRYPT_KEY", "")
            if not feishu_key:
                logger.warning("[webhook] FEISHU_ENCRYPT_KEY not configured")
                return False
            # Feishu uses SHA256 (not HMAC) of timestamp + nonce + encrypt_key + body
            content = (lark_ts + lark_nonce + feishu_key).encode("utf-8") + body
            expected_feishu = hashlib.sha256(content).hexdigest()
            return _hmac_str_equal(lark_sig, expected_feishu)
    ...
    async def _deliver_cross_platform(
        ...
        *,
        ...
        # Merge caller metadata (the stream consumer tags first sends with
        # ``expect_edits``/``notify`` so the feishu adapter can render the
        # main reply as a Card Kit v2 streaming card) with the thread_id from
        # deliver_extra so Telegram forum topics still work.
        target_meta = dict(metadata or {})
        thread_id = extra.get("message_thread_id") or extra.get("thread_id")
        if thread_id:
            target_meta.setdefault("thread_id", thread_id)
        target_meta = target_meta or None

        return await adapter.send(
            chat_id, content, metadata=target_meta
        )
    ...
    async def edit_message(
        ...
        *,
        ...
        """Proxy streaming edits to the cross-platform target adapter.

        The gateway's stream consumer drives the webhook adapter for these
        routes, so without a proxy its per-frame ``edit_message`` calls would
        fail and the consumer would fall back to re-sending whole messages.
        Delegate to the target adapter (e.g. feishu) which edits its card in
        place on each frame.
        """
        delivery = self._delivery_info.get(chat_id, {})
        deliver_type = delivery.get("deliver", "log")
        ...
        if not self.gateway_runner or not _is_known_platform:
            return SendResult(
                success=False, error=f"Unsupported deliver type: {deliver_type}"
            )
        ...
        try:
            ...
            return SendResult(
                success=False, error=f"Unknown platform: {deliver_type}"
            )
        ...
        if not adapter:
            return SendResult(
                success=False,
                error=f"Platform {deliver_type} not connected",
            )

        target_chat_id = chat_id
        extra = delivery.get("deliver_extra", {})
        extra_chat_id = extra.get("chat_id", "")
        if extra_chat_id:
            target_chat_id = extra_chat_id

        edit = getattr(adapter, "edit_message", None)
        if edit is None:
            return SendResult(
                success=False, error=f"{deliver_type} does not support edits"
            )
        return await edit(
            target_chat_id, str(message_id), content, finalize=finalize
        )
