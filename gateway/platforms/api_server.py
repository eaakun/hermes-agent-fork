...
from datetime import datetime, timedelta, timezone  # [2026-09-04] cost dashboard
...
def verify_dispatch_hmac(secret: str, signature_header: str, raw_body: bytes) -> bool:
    """[2026-09-08] Regression for 1bot task dispatch HMAC prefix compatibility.

    The 1bot task_dispatcher signs payloads in two flavors depending on
    version:
      - ``sha256=<hex>`` (newer, RFC-9421-style)
      - raw lowercase hex (older, raw hmac.new().hexdigest())

    Both must validate. The header comparison is case-insensitive on the
    algorithm prefix and the hex digest. Returns True iff ``signature_header``
    matches HMAC-SHA256(secret, raw_body) under either accepted encoding.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            # [2026-09-04] Cost dashboard: 查询每日 LLM token 消耗 (dispatch 端点累加写入)
            ("GET", "/api/cost/today", self._handle_cost_today),
            ("GET", "/api/cost/range", self._handle_cost_range),
            ("POST", "/api/cost/cleanup", self._handle_cost_cleanup),
            ("GET", "/api/context/checkpoints", self._handle_context_checkpoints),
            ...
            # 1bot relay: authenticated by HMAC shared secret in X-1bot-Signature header
            ("POST", "/relay/1bot-send", self._handle_relay_1bot_send),
            # [2026-09-02 方案 C+6 阶段 2] 1bot LLM 任务派发
            ("POST", "/api/dispatch/task", self._handle_dispatch_task),
            # [2026-09-08] 1bot 读图: 用 image_key 下载图片, 调 vision_analyze_tool
            # 拿描述返给 1bot. 1bot 把描述拼到 prompt 再走 /api/dispatch/task.
            ("POST", "/api/vision/analyze", self._handle_vision_analyze),
        ]
        if _CRON_AVAILABLE:
    ...
    async def _handle_relay_1bot_send(self, request: "web.Request") -> "web.Response":
        """POST /relay/1bot-send — 1bot calls this to send Feishu messages via Hermes app.

        Authenticated by HMAC-SHA256 in X-1bot-Signature header (shared secret
        via GATEWAY_RELAY_SHARED_KEY env var, same mechanism as existing webhook HMAC).
        Body: JSON {chat_id, content, msg_type, reply_to_message_id}
        Sends via the Feishu adapter so the Hermes app's token is used (the only
        app that can message the user's DM oc_302628...).
        """
        # 1. Read body
        ...
        if not chat_id or not content:
            return web.json_response({"error": "chat_id and content are required"}, status=400)

        # 2. Verify HMAC (same secret as Hermes webhook routes — shared with 1bot)
        secret = os.environ.get("GATEWAY_RELAY_SHARED_KEY", "")
        if secret:
            sig = request.headers.get("X-1bot-Signature", "")
            if not sig:
                return web.json_response({"error": "Missing X-1bot-Signature header"}, status=401)
            raw = await request.read()
            expected = hmac.new(secret.encode(), raw, "sha256").hexdigest()
            if not hmac.compare_digest(sig.lower(), expected):
                return web.json_response({"error": "Invalid signature"}, status=401)

        # 3. Get Feishu adapter
        feishu = self._get_platform_callback_adapter(request, "feishu")
        if feishu is None:
            return web.json_response({"error": "Feishu adapter not available"}, status=503)

        try:
            if msg_type == "card":
                result = await feishu.send_card(chat_id, content, reply_to=reply_to)
            else:
                result = await feishu.send(chat_id, content, reply_to=reply_to)
            if result.success:
                return web.json_response({"ok": True, "message_id": result.message_id})
            return web.json_response({"error": result.error or "send failed"}, status=502)
        except Exception as e:
            logger.exception("[relay-1bot] send failed")
            return web.json_response({"error": str(e)}, status=500)

    # ------------------------------------------------------------------
    # ── 1bot task dispatch (方案 C+6 阶段 2) ───────────────────────────
    async def _handle_dispatch_task(self, request: "web.Request") -> "web.Response":
        """POST /api/dispatch/task — 1bot 把 LLM 任务交给 Hermes 跑.

        Auth: HMAC-SHA256 in X-Signature header (shared secret via
              GATEWAY_RELAY_SHARED_KEY env var).

        Body: {
          task_id, text, sender_id, chat_id, source ("1bot"),
          callback_url (可选, 异步回传),
          system_prompt (可选, 覆盖默认)
        }

        Returns: {
          ok, task_id, reply, session_id, model, latency_ms
        }

        注: 本端点是方案 C+6 阶段 2 产物. 1bot 不再自己调 opencode subprocess,
        改由 1bot task_dispatcher.py POST 这里. 1bot webhook_server 重启后
        不再调 acp_client.prompt, 只调 task_dispatcher.dispatch().
        """
        # 1. Read body
        try:
            raw = await request.read()
        except Exception as e:
            return web.json_response({"error": f"read body failed: {e}"}, status=400)

        # 2. Verify HMAC
        secret = os.environ.get("GATEWAY_RELAY_SHARED_KEY", "")
        if not secret:
            return web.json_response({"error": "GATEWAY_RELAY_SHARED_KEY not configured"}, status=503)
        sig = request.headers.get("X-Signature", "")
        if not sig:
            return web.json_response({"error": "Missing X-Signature header"}, status=401)
        # 兼容两种格式: "sha256=<hex>" (1bot task_dispatcher 风格) 或纯 hex
        if not verify_dispatch_hmac(secret, sig, raw):
            return web.json_response({"error": "Invalid signature"}, status=401)
        ...
        task_id = body.get("task_id") or f"1bot_{int(time.time()*1000)}"
        text = (body.get("text") or "").strip()
        sender_id = body.get("sender_id", "1bot-user")
        chat_id = body.get("chat_id", "")
        source = body.get("source", "1bot")
        system_prompt = body.get("system_prompt")
        # [2026-09-07] 飞书 thread 上下文透传 — parent_id 缺失会断 thread 上文
        # 1bot webhook_server 发 {text, chat_id, message_id, reply_to_message_id} 时,
        # message_id 即 parent_id. Hermes 端透传到 _run_agent 作为 thread 锚点.
        parent_id = body.get("parent_id") or body.get("reply_to_message_id") or body.get("message_id") or ""
        reply_to_message_id = body.get("reply_to_message_id") or ""
        ...
        # [2026-09-08] 1bot DM skills + 记忆升级: 不再覆盖 system prompt, 改为 message
        # 前缀注入 1bot 专属 persona. 保留 Hermes 默认 skills 索引 + 上下文压缩 + 持久 session.
        # 让 LLM 自动加载 skills (cronjob/feishu-webhook-traps/hermes-gateway-config 等),
        # 与 Hermes DM 用户体验一致.
        if not system_prompt:
            user_message_with_persona = (
                "[1bot DM 上下文 — 你是 1bot 助手, 正在飞书 DM 处理用户消息. "
                "用户用飞书聊天, 回复简洁直接 (≤ 500 字), 用中文.]\n\n"
                + text
            )
            ephemeral_prompt_to_use = None  # 不覆盖 system, 保留 skills
        else:
            user_message_with_persona = text
            ephemeral_prompt_to_use = system_prompt

        # [2026-09-09] 1bot 读图: 1bot 透传 image_key 进来, Hermes 端下载并调 vision_analyze,
        # 把描述 prepend 到 text, 让 LLM 看到真正的图. 1bot 本身不调 vision 接口,
        # 因为 LLM 只看 text. 这一步把 image_key → description → LLM 输入打通.
        image_key = body.get("image_key", "").strip()
        if image_key:
            logger.info(f"[dispatch] vision enrich start image_key={image_key[:24]}")
            # 直接裸 HTTP 拿 tenant_access_token — 不依赖 import 路径
            _app_id = os.environ.get("FEISHU_APP_ID", "").strip()
            _app_secret = os.environ.get("FEISHU_APP_SECRET", "").strip()
            _domain = os.environ.get("FEISHU_DOMAIN", "feishu").strip().lower() or "feishu"
            _tenant_token = ""
            if _app_id and _app_secret:
                try:
                    import urllib.request as _ur_tok, urllib.parse as _up
                    _base = "https://open.feishu.cn" if _domain == "feishu" else "https://open.larksuite.com"
                    _tok_body = json.dumps({"app_id": _app_id, "app_secret": _app_secret}).encode()
                    _tok_req = _ur_tok.Request(
                        f"{_base}/open-apis/auth/v3/tenant_access_token/internal",
                        data=_tok_body,
                        headers={"Content-Type": "application/json"},
                    )
                    with _ur_tok.urlopen(_tok_req, timeout=10) as _tr:
                        _tok_res = json.loads(_tr.read().decode())
                    _tenant_token = _tok_res.get("tenant_access_token", "")
                except Exception as _tok_exc:
                    logger.warning(f"[dispatch] tenant_token fetch failed: {_tok_exc}")
            if _tenant_token:
                try:
                    import urllib.request as _ur
                    _img_url = (f"https://open.feishu.cn/open-apis/im/v1/messages/"
                                f"{image_key}/resources/image?type=image")
                    with _ur.urlopen(_ur.Request(_img_url, headers={"Authorization": f"Bearer {_tenant_token}"}), timeout=30) as _r:
                        _img_bytes = _r.read()
                    from pathlib import Path as _P
                    _cache = _P("/Users/eaakun/.hermes/cache/vision")
                    _cache.mkdir(parents=True, exist_ok=True)
                    _local = _cache / f"1bot_{image_key[:24]}.jpg"
                    _local.write_bytes(_img_bytes)
                    from tools.vision_tools import vision_analyze_tool
                    _v_model = ""
                    try:
                        from hermes_cli.config import load_config, cfg_get
                        _cfg = load_config()
                        _v_model = cfg_get(_cfg, "auxiliary", "vision", "model", default="")
                        if not _v_model:
                            _v_provider = cfg_get(_cfg, "auxiliary", "vision", "provider", default="")
                            _v_model = cfg_get(_cfg, "providers", _v_provider, "model", default="")
                    except Exception:
                        pass
                    _v_result = json.loads(await vision_analyze_tool(
                        image_url=str(_local),
                        user_prompt="Describe everything visible in this image in thorough detail. Reply in Chinese.",
                        model=_v_model or None,
                    ))
                    if _v_result.get("success"):
                        _desc = _v_result.get("analysis", "").strip()
                        if _desc:
                            text = f"[图片描述: {_desc}]\n\n{text}"
                            logger.info(f"[dispatch] vision enrich image_key={image_key[:24]} chars={len(_desc)}")
                    else:
                        logger.warning(f"[dispatch] vision failed: {_v_result.get('analysis', 'unknown')[:100]}")
                except Exception as _v_exc:
                    logger.warning(f"[dispatch] vision enrich error: {_v_exc}")
            else:
                logger.warning(f"[dispatch] vision skipped image_key={image_key[:24]} tenant_token empty (FEISHU_APP_ID={'yes' if _app_id else 'no'})")

        # 3b. 重新组装 persona + (可能 enrich 过的) text
        # [2026-09-10] 修 bug: vision enrich 后 text 变了, 但 user_message_with_persona
        # 是在 persona 块 (L2782) 用旧 text 生成的. 这里按"是否有 system_prompt"重新算:
        if not system_prompt:
            user_message_with_persona = (
                "[1bot DM 上下文 — 你是 1bot 助手, 正在飞书 DM 处理用户消息. "
                "用户用飞书聊天, 回复简洁直接 (≤ 500 字), 用中文.]\n\n"
        ...
        # 4. Dispatch
        logger.info(f"[dispatch] task_id={task_id} chat_id={chat_id} parent_id={parent_id[:24] if parent_id else ''} source={source} text={text[:60]}...")
        t0 = time.time()
        try:
            result, usage = await self._run_agent(
                user_message=user_message_with_persona,
                conversation_history=[],
                ephemeral_system_prompt=ephemeral_prompt_to_use,
                agent_ref=None,
                # 1bot session 持久化: 用 1bot-{chat_id} 作为 session_key,
                # 让 LLM 跨调用能拉到同一 session 历史 (走 gateway session 机制)
                gateway_session_key=f"1bot-{chat_id}",
                # session_id 也传, 让记忆/上下文压缩能跨 turn 复用
                session_id=f"1bot-{chat_id}",
            )
            latency_ms = int((time.time() - t0) * 1000)
            reply = result.get("reply") or result.get("text") or str(result)
            model = result.get("model", "unknown")
            logger.info(f"[dispatch] task_id={task_id} OK in {latency_ms}ms ({len(reply)} chars)")

            # 9/4 三层防护第 3 层: 无输出可见性 — reply 为空时给用户一行状态,避免沉默掉链子
            if not (reply and reply.strip()):
                try:
                    from gateway.lib.context_checkpoint import empty_response_marker
                    reply = empty_response_marker(
                        final_response=reply,
                        elapsed_sec=latency_ms / 1000.0,
                        usage=usage,
                    )
                    logger.info(f"[dispatch] task_id={task_id} empty reply replaced with status marker")
                except Exception as _mk_exc:  # pragma: no cover
                    logger.debug("empty marker skipped: %s", _mk_exc)

            # [2026-09-04] 累加 token 消耗到 daily cost 文件 (即使 M2 free 也算 shadow cost)
            try:
                import json as _j, os as _o
                from datetime import datetime as _dt, timezone as _tz
                cost_dir = _o.path.expanduser("~/.hermes/cost")
                _o.makedirs(cost_dir, exist_ok=True)
                today = _dt.now(_tz.utc).astimezone().strftime("%Y-%m-%d")
                cost_file = _o.path.join(cost_dir, "daily.json")
                cost_data = {}
                if _o.path.exists(cost_file):
                    try:
                        cost_data = _j.loads(open(cost_file).read())
                    except Exception:
                        cost_data = {}
                if today not in cost_data:
                    cost_data[today] = {"calls": 0, "input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "by_model": {}, "by_source": {}}
                day = cost_data[today]
                day["calls"] += 1
                in_t = (usage or {}).get("input_tokens", 0)
                out_t = (usage or {}).get("output_tokens", 0)
                tot_t = (usage or {}).get("total_tokens", in_t + out_t)
                day["input_tokens"] += in_t
                day["output_tokens"] += out_t
                day["total_tokens"] += tot_t
                # by model
                m = day["by_model"].setdefault(model, {"input_tokens": 0, "output_tokens": 0, "calls": 0})
                m["input_tokens"] += in_t
                m["output_tokens"] += out_t
                m["calls"] += 1
                # by source (1bot-repair / 1bot / 飞书等)
                src = body.get("source", "unknown")
                s = day["by_source"].setdefault(src, {"input_tokens": 0, "output_tokens": 0, "calls": 0})
                s["input_tokens"] += in_t
                s["output_tokens"] += out_t
                s["calls"] += 1
                # 写回 (锁文件避免并发写竞态)
                lock_file = cost_file + ".lock"
                lock_fd = _o.open(lock_file, _o.O_CREAT, 0o644)
                try:
                    import fcntl as _fctl
                    _fctl.flock(lock_fd, _fctl.LOCK_EX)
                    open(cost_file, "w").write(_j.dumps(cost_data, ensure_ascii=False, indent=2))
                finally:
                    _fctl.flock(lock_fd, _fctl.LOCK_UN)
                    lock_fd.close()
            except Exception as cost_err:
                logger.warning(f"[dispatch] cost tracking failed: {cost_err}")

            callback_url = body.get("callback_url")
            if callback_url:
                asyncio.create_task(self._dispatch_callback(callback_url, {
                    "task_id": task_id,
                    "ok": True,
                    "reply": reply,
            ...
            logger.exception(f"[dispatch] task_id={task_id} failed")
            return web.json_response({
                "ok": False,
                "task_id": task_id,
                "error": str(e),
    ...
    async def _dispatch_callback(self, url: str, payload: Dict[str, Any]):
        """异步 POST 任务结果回 1bot callback_url (best-effort)."""
        try:
            data = json.dumps(payload, ensure_ascii=False).encode("utf-8")
            sig = hmac.new(
                os.environ.get("GATEWAY_RELAY_SHARED_KEY", "").encode(),
                data, "sha256",
            ).hexdigest()
            ...
            with urllib.request.urlopen(req, timeout=10) as r:
                logger.info(f"[dispatch-callback] {url} -> {r.status}")
        except Exception as e:
            logger.warning(f"[dispatch-callback] {url} failed: {e}")

    # ------------------------------------------------------------------
    # ── 1bot 读图 (2026-09-08 Trap 18) ───────────────────────────────
    async def _handle_vision_analyze(self, request: "web.Request") -> "web.Response":
        """POST /api/vision/analyze — 1bot 发 image_key (或 image_url), Hermes 下载图片
        调 vision_analyze_tool 拿描述, 返 {ok, description, image_url}.

        Auth: HMAC-SHA256 in X-Signature (同 /api/dispatch/task).
        Body: {
          image_key: str (飞书图片的 image_key), OR
          image_url: str (http(s) URL, 任意可访问的图片),
          user_prompt: str (可选, 默认 "Describe this image in detail.")
          tenant_token: str (可选, 用 1bot 自己的 token 下载飞书图片;
                            没传就走 feishu_bridge.get_token)
        }

        1bot 用法:
          1. 收到用户图片 → /api/vision/analyze {image_key: "xxx", tenant_token: "..."}
          2. 拿 description → 拼到 prompt: "[图片描述] " + 后续文字
          3. /api/dispatch/task {text: enriched_text, ...}
        """
        # 1. Read body
        try:
            raw = await request.read()
        ...
        user_prompt = body.get("user_prompt") or (
            ...
            "and any other notable visual information. Reply in Chinese."
        )
        tenant_token = body.get("tenant_token", "").strip()

        if not image_key and not image_url:
            return web.json_response({"error": "image_key or image_url required"}, status=400)

        # 4. 下载图片到本地 (如果 image_key, 从飞书下载; 如果 image_url, 用 urllib 下载)
        from pathlib import Path as _P
        cache_dir = _P("/Users/eaakun/.hermes/cache/vision")
        cache_dir.mkdir(parents=True, exist_ok=True)
        local_path = None
        try:
            if image_key:
                # 下载飞书图片. 用 tenant_token (1bot 传的) 或自己取 token.
                if not tenant_token:
                    try:
                        from feishu_bridge import get_token as _gt
                        tenant_token = _gt()
                    except Exception:
                        pass
                if not tenant_token:
                    return web.json_response(
                        {"error": "tenant_token required (1bot 没传, Hermes 端也没拿不到)"}, status=400)
                ...
                with urllib.request.urlopen(req2, timeout=30) as r2:
                    img_bytes = r2.read()
                tmp = cache_dir / f"1bot_{image_key[:24]}.jpg"
                tmp.write_bytes(img_bytes)
                local_path = str(tmp)
            else:
                # 直接用 urllib 下 image_url
                req2 = urllib.request.Request(image_url)
                with urllib.request.urlopen(req2, timeout=30) as r2:
                    img_bytes = r2.read()
                ext = ".jpg"
                if image_url.lower().endswith(".png"): ext = ".png"
                elif image_url.lower().endswith(".gif"): ext = ".gif"
                elif image_url.lower().endswith(".webp"): ext = ".webp"
                tmp = cache_dir / f"vision_{int(time.time()*1000)}{ext}"
                tmp.write_bytes(img_bytes)
                local_path = str(tmp)
        except Exception as e:
            logger.warning(f"[vision/analyze] download failed: {e}")
            return web.json_response({"error": f"download failed: {e}"}, status=502)

        # 5. 调 vision_analyze_tool 拿描述
        try:
            from tools.vision_tools import vision_analyze_tool
            t0 = time.time()
            # [2026-09-08] 显式读 providers.huggingface.model (auxiliary.vision.model 通常空),
            # 让 vision_analyze_tool 把 model 传给 async_call_llm(task="vision"),
            # 后者会自动从 auxiliary.vision.provider=huggingface 读 provider.
            _vision_model = ""
            try:
                from hermes_cli.config import load_config, cfg_get
                _cfg = load_config()
                _vision_model = cfg_get(_cfg, "auxiliary", "vision", "model", default="")
                if not _vision_model:
                    _vision_provider = cfg_get(_cfg, "auxiliary", "vision", "provider", default="")
                    _vision_model = cfg_get(_cfg, "providers", _vision_provider, "model", default="")
            except Exception:
                pass
            result_json = await vision_analyze_tool(
                image_url=local_path,
                user_prompt=user_prompt,
                model=_vision_model or None,
            )
            latency_ms = int((time.time() - t0) * 1000)
            result = json.loads(result_json)
            if not result.get("success"):
                return web.json_response(
                    {"ok": False, "error": result.get("analysis", "vision failed"),
                     "image_url": local_path}, status=500)
            description = result.get("analysis", "")
            logger.info(f"[vision/analyze] OK in {latency_ms}ms ({len(description)} chars) path={local_path}")
            return web.json_response({
                "ok": True,
            ...
            # local 文件留给后续 vision_analyze 复用, 不立刻删
            pass
    ...
    async def _handle_context_checkpoints(self, request: "web.Request") -> "web.Response":
        """GET /api/context/checkpoints?session_id=X&limit=N — 拉 checkpoint 历史.

        [2026-09-06] 截断感知防护层第 2 层配套端点.
        用法: 新会话 /new 后,如果怀疑之前被截断,调这里看上次 handoff 摘要.
        """
        try:
            from gateway.lib.context_checkpoint import recent_checkpoints_for_session
            sid = request.query.get("session_id", "").strip()
            try:
            ...
            if not sid:
                return web.json_response(
                    {"error": "session_id query param required"}, status=400
                )
            rows = recent_checkpoints_for_session(sid, limit=limit)
            return web.json_response({"ok": True, "session_id": sid, "count": len(rows), "checkpoints": rows})
        except Exception as exc:
            logger.warning("context_checkpoints query failed: %s", exc)
            return web.json_response({"error": f"failed: {exc}"}, status=500)

    # [2026-09-04] Cost tracking helper — atomic append to ~/.hermes/cost/daily.json
    def _record_cost(self, usage: dict, model: str, source: str) -> None:
        """Record LLM usage to daily cost file. Atomic via temp file + rename.
        Best-effort: failures are swallowed (cost tracking must never break a request)."""
        if not usage:
            return
        ...
        if in_t == 0 and out_t == 0:
            return
        try:
            cost_file = os.path.expanduser("~/.hermes/cost/daily.json")
            # Load existing
            try:
            ...
            day = data.get(today, {"calls": 0, "input_tokens": 0, "output_tokens": 0,
                                    "by_model": {}, "by_source": {}})
            day["calls"] += 1
            day["input_tokens"] += in_t
            day["output_tokens"] += out_t
            bm = day.setdefault("by_model", {})
            m = bm.setdefault(model or "unknown", {"calls": 0, "input_tokens": 0, "output_tokens": 0})
            m["calls"] += 1
            m["input_tokens"] += in_t
            m["output_tokens"] += out_t
            bs = day.setdefault("by_source", {})
            s = bs.setdefault(source or "unknown", {"calls": 0, "input_tokens": 0, "output_tokens": 0})
            s["calls"] += 1
            s["input_tokens"] += in_t
            s["output_tokens"] += out_t
            data[today] = day
            ...
            logger.debug("cost record failed (non-fatal): %s", e)

    # [2026-09-04] Cost dashboard: query endpoints
    # [2026-09-04] Cost dashboard: per-model pricing (shadow USD per 1M tokens)
    # Free models (suffix :free) get 0 cost. Real models get estimated pricing.
    # Override or extend in ~/.hermes/cost/model_pricing.json if needed.
    _MODEL_PRICING = {
        "minimax/minimax-m3:free":  {"input": 0.0,  "output": 0.0},   # free tier
        "minimax/minimax-m2:free":  {"input": 0.0,  "output": 0.0},
        "minimax/minimax-m2.7":     {"input": 0.5,  "output": 2.0},   # M2 (default)
        "minimax/minimax-m2.5":     {"input": 0.4,  "output": 1.6},
        "minimax/minimax-m2":       {"input": 0.5,  "output": 2.0},
    ...
    _DEFAULT_PRICING = {"input": 0.5, "output": 2.0}  # fallback

    def _model_pricing(self, model: str) -> dict:
        """Return $/1M-token pricing for a model. Handles :free suffix and unknown models."""
        if not model:
            return self._DEFAULT_PRICING
        # Anything with ':free' suffix is free
        if model.endswith(":free"):
            return {"input": 0.0, "output": 0.0}
        if model in self._MODEL_PRICING:
            return self._MODEL_PRICING[model]
        # Try fuzzy match on prefix (e.g. 'minimax/...' or 'anthropic/...')
        for k, v in self._MODEL_PRICING.items():
            if model.startswith(k.split("/")[-1]) or k.startswith(model):
                return v
        return self._DEFAULT_PRICING

    def _calc_model_cost(self, model: str, in_t: int, out_t: int) -> float:
        p = self._model_pricing(model)
        return round(in_t * p["input"] * 1e-6 + out_t * p["output"] * 1e-6, 4)

    async def _handle_cost_today(self, request: "web.Request") -> "web.Response":
        """GET /api/cost/today — today's LLM usage with per-model cost."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cost_file = os.path.expanduser("~/.hermes/cost/daily.json")
        ...
        try:
            ...
            return web.json_response({"error": str(e)}, status=500)
        day = data.get(today, {"calls": 0, "input_tokens": 0, "output_tokens": 0})
        in_t = day.get("input_tokens", 0)
        out_t = day.get("output_tokens", 0)
        shadow = round(in_t * 0.5e-6 + out_t * 2e-6, 4)
        # [2026-09-04] Per-model cost breakdown
        by_model_raw = day.get("by_model", {})
        by_model_cost = {}
        model_cost_total = 0.0
        for m, stats in by_model_raw.items():
            m_in = stats.get("input_tokens", 0)
            m_out = stats.get("output_tokens", 0)
            m_cost = self._calc_model_cost(m, m_in, m_out)
            by_model_cost[m] = {
                **stats,
                "shadow_cost_usd": m_cost,
            }
            model_cost_total += m_cost
        return web.json_response({
            "date": today,
            ...
            "shadow_cost_by_model_usd": round(model_cost_total, 4),
            "by_model": by_model_cost,
            "by_source": day.get("by_source", {}),
        })

    async def _handle_cost_range(self, request: "web.Request") -> "web.Response":
        """GET /api/cost/range?days=7 — N-day LLM usage summary."""
        cost_file = os.path.expanduser("~/.hermes/cost/daily.json")
        try:
        ...
        # [2026-09-04] Aggregate per-model across the window
        model_agg: dict = {}  # model -> {calls, in, out}
        for i in range(days):
            d = (today - timedelta(days=i)).strftime("%Y-%m-%d")
            day = data.get(d, {"calls": 0, "input_tokens": 0, "output_tokens": 0})
            in_t = day.get("input_tokens", 0)
            out_t = day.get("output_tokens", 0)
            days_list.append({
                "date": d,
                ...
                "output_tokens": out_t,
                "total_tokens": in_t + out_t,
                "shadow_cost_usd": round(in_t * 0.5e-6 + out_t * 2e-6, 4),
            })
            totals["calls"] += day.get("calls", 0)
            totals["input_tokens"] += in_t
            totals["output_tokens"] += out_t
            for m, stats in day.get("by_model", {}).items():
                agg = model_agg.setdefault(m, {"calls": 0, "input_tokens": 0, "output_tokens": 0})
                agg["calls"] += stats.get("calls", 0)
                agg["input_tokens"] += stats.get("input_tokens", 0)
                agg["output_tokens"] += stats.get("output_tokens", 0)
        totals["shadow_cost_usd"] = round(
            totals["input_tokens"] * 0.5e-6 + totals["output_tokens"] * 2e-6, 4
        )
        # [2026-09-04] Per-model cost aggregation
        by_model_cost = {}
        model_cost_total = 0.0
        for m, agg in model_agg.items():
            mc = self._calc_model_cost(m, agg["input_tokens"], agg["output_tokens"])
            by_model_cost[m] = {
                **agg,
                "shadow_cost_usd": mc,
            }
    ...
    async def _handle_cost_cleanup(self, request: "web.Request") -> "web.Response":
        """POST /api/cost/cleanup?older_than_days=N — prune daily cost data older than N days.
        Body (optional): {"dry_run": true, "keep_min_days": 7}.
        Default: removes entries older than 90 days, but never removes the last `keep_min_days` (default 30).
        Query ?asof=YYYY-MM-DD (testing) overrides "today" for preview month-end logic.
        """
        try:
            older_than = int(request.query.get("older_than_days", "90"))
            older_than = max(1, min(older_than, 3650))
        except (TypeError, ValueError):
            older_than = 90
        ...
        keep_min_days = max(1, int(body.get("keep_min_days", 30)))
        ...
        # [2026-09-04] Test override: ?asof=YYYY-MM-DD lets cron tests verify day>=28 preview path
        today = datetime.now(timezone.utc).astimezone()
        asof = request.query.get("asof")
        if asof:
            try:
                today = datetime.fromisoformat(asof).replace(tzinfo=today.tzinfo)
            except (TypeError, ValueError):
                pass
        cutoff = (today - timedelta(days=older_than)).strftime("%Y-%m-%d")
        protected_floor = (today - timedelta(days=keep_min_days)).strftime("%Y-%m-%d")

        removed = []
        kept = []
        for date_key in sorted(data.keys()):
            if date_key < cutoff and date_key < protected_floor:
                removed.append(date_key)
            else:
                kept.append(date_key)

        if not dry_run and removed:
            new_data = {k: v for k, v in data.items() if k not in removed}
            tmp = cost_file + ".tmp"
            with open(tmp, "w", encoding="utf-8") as f:
                f.write(json.dumps(new_data, ensure_ascii=False))
            os.replace(tmp, cost_file)

        # [2026-09-04] Monthly preview: 当今天 >= 28 号且本月尚未预览过，附加 end-of-month 预览块
        # 这样月度清理 cron 不会"悄无声息"删东西——推送飞书时附带预览
        preview_block = None
        today_date = today.date() if hasattr(today, 'date') else today
        if today_date.day >= 28:
            preview_block = {
                "note": "month-end preview (today >= 28)",
                "would_remove": removed,
                "would_keep": kept,
                "older_than_days": older_than,
                "keep_min_days": keep_min_days,
                "cutoff_date": cutoff,
                "protected_floor_date": protected_floor,
            }
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        # 9/4 三层防护第 2 层: 截断感知 — session 内 history 含 CONTEXT COMPACTION handoff 时落盘
        try:
            from gateway.lib.context_checkpoint import maybe_checkpoint
            maybe_checkpoint(
                session_id=session_id,
                history=history,
            ...
            logger.debug("checkpoint scan skipped: %s", _cp_exc)
        ...
        # [2026-09-04] Cost tracking: record usage for /api/sessions/{id}/chat
        try:
            _model = (result.get("model") if isinstance(result, dict) else None) or "unknown"
            self._record_cost(usage, _model, "session_chat")
        except Exception:
            pass
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        async def _run_and_signal() -> None:
            try:
                ...
                # [2026-09-04] Cost tracking: record usage for /api/sessions/{id}/chat/stream
                try:
                    _model = (result.get("model") if isinstance(result, dict) else None) or "unknown"
                    self._record_cost(usage, _model, "session_chat_stream")
                except Exception:
                    pass
                final_response = _resolve_media_to_data_urls(result.get("final_response", "") if isinstance(result, dict) else "")
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # 9/4 三层防护第 2 层: 截断感知 — 检测 history 里 CONTEXT COMPACTION handoff
        try:
            from gateway.lib.context_checkpoint import maybe_checkpoint
            maybe_checkpoint(
                session_id=request.headers.get("X-Hermes-Session-Id", "") or request.headers.get("X-Hermes-Session-Key", ""),
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if idempotency_key:
            ...
            # [2026-09-04] Cost tracking: record usage for /v1/responses (cached path)
            try:
                _model = (result.get("model") if isinstance(result, dict) else None) or "unknown"
                self._record_cost(usage, _model, "v1_responses")
            except Exception:
                pass
        else:
            try:
            ...
            # [2026-09-04] Cost tracking: record usage for /v1/responses (direct path)
            try:
                _model = (result.get("model") if isinstance(result, dict) else None) or "unknown"
                self._record_cost(usage, _model, "v1_responses")
            except Exception:
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                ...
                # [2026-09-04] Cost tracking: record usage for /v1/runs (background agent)
                try:
                    _model = (result.get("model") if isinstance(result, dict) else None) or "unknown"
                    self._record_cost(usage, _model, "v1_runs")
                except Exception:
                    pass
                if run_id in self._stopping_run_ids:
                    _put_event_if_active({
