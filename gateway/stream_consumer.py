...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        on_inspiration: Optional[Callable[[str], None]] = None,
        ...
        # Fired when a <inspiration>...</inspiration> tag is fully extracted
        # from the streamed response. The gateway binds this to the inspiration
        # queue dispatcher. Exceptions are swallowed.
        ...
        # Inspiration tag buffer: holds partial <inspiration>...</inspiration>
        # content that hasn't been closed yet.
        self._inspiration_hold = ""
    ...
    # Inspiration tag: a signal injected by the LLM into its output to share
    # a spontaneous insight with the user asynchronously.
    _INSPIRATION_RE = re.compile(
        r"<inspiration>(.*?)</inspiration>", re.DOTALL | re.IGNORECASE
    )

    @staticmethod
    def _extract_inspirations(text: str) -> list[str]:
        """Extract all <inspiration>...</inspiration> tags from text.

        Returns a list of captured contents. The tags themselves are removed
        from the text so they never appear in the displayed message.
        """
        matches = GatewayStreamConsumer._INSPIRATION_RE.findall(text)
        ...
        while buf:
            ...
            if self._in_think_block:
                ...
                if best_len:
                    ...
                    # ── Inspiration tag extraction ────────────────────────────
                    # Strip <inspiration>...</inspiration> tags from displayed
                    # output and fire async callbacks for each complete tag.
                    if "</inspiration>" in buf.lower():
                        buf = self._process_inspiration_tags(buf)
                    else:
                        # No closing tag yet — hold partial content for next chunk
                        self._inspiration_hold += buf
                        return

    def _process_inspiration_tags(self, text: str) -> str:
        """Strip <inspiration> tags from *text*, firing callbacks for each complete tag.

        Partial tags at the buffer boundary are held in ``_inspiration_hold``.
        Returns the text with all complete tags removed.
        """
        combined = self._inspiration_hold + text
        self._inspiration_hold = ""

        while True:
            match = self._INSPIRATION_RE.search(combined)
            if not match:
                break
            content = match.group(1).strip()
            if content and self._on_inspiration:
                try:
                    # Schedule as fire-and-forget so the filter loop is not blocked.
                    # The callback must be sync-callable; if it needs to do async
                    # work (e.g. adapter.send) it should schedule its own task.
                    self._loop.call_soon_threadsafe(self._on_inspiration, content)
                except Exception:
                    pass
            combined = combined[:match.start()] + combined[match.end():]

        # If combined ends with an unclosed <inspiration>, hold the partial
        lower = combined.lower()
        open_pos = lower.rfind("<inspiration>")
        if open_pos > lower.rfind("</inspiration>") if "</inspiration>" in lower else True:
            # Unclosed tag at tail — hold content after it
            self._inspiration_hold = combined[open_pos:]
            return combined[:open_pos]
