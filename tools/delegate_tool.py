...
def _classify_stop_reason(exit_reason: str, status: str) -> str:
    """Collapse fine-grained exit reasons into canonical replay categories.

    Canonical taxonomy (for Harness replay / cross-version drift measurement):
      goal_achieved      — subagent produced usable output and stopped on its own
      budget_exhausted   — iteration/token budget hit (max_iterations, budget_exhausted)
      timeout            — external timeout fired
      tool_error         — terminal tool/processing failure
      context_overflow   — context window exhausted / ollama too-small / retries
      interrupted        — user or parent interrupted
      unknown            — no usable signal

    Maps both the coarse exit_reason set (completed/max_iterations/interrupted/
    timeout/error) and conversation_loop's fine-grained _turn_exit_reason values.
    """
    er = str(exit_reason or "").strip()
    st = str(status or "").strip()

    if er == "completed" or er.startswith("text_response(") or er == "unknown" and st == "completed":
        return "goal_achieved"
    if er in ("interrupted", "interrupted_by_user", "interrupted_during_api_call"):
        return "interrupted"
    if er in ("timeout",):
        return "timeout"
    if er in ("budget_exhausted", "max_iterations", "error_near_max_iterations"):
        return "budget_exhausted"
    if er in ("local_processing_error",) or er == "error":
        return "tool_error"
    if er in ("context_overflow", "ollama_runtime_context_too_small", "context_length_exceeded"):
        return "context_overflow"
    if er in ("empty_response_exhausted", "all_retries_exhausted_no_response",
              "partial_stream_recovery", "fallback_prior_turn_content", "guardrail_halt"):
        return "tool_error"
    if st == "completed":
        return "goal_achieved"
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        # Prefer the subagent's own fine-grained _turn_exit_reason (conversation_loop)
        # over the coarse interrupted/completed bools. conversation_loop records
        # budget_exhausted, interrupted_by_user, all_retries_exhausted_no_response,
        # ollama_runtime_context_too_small, empty_response_exhausted, etc.
        fine_reason = result.get("_turn_exit_reason") or ""
        if interrupted:
            exit_reason = "interrupted"
        elif fine_reason and isinstance(fine_reason, str):
            exit_reason = fine_reason
        elif completed:
        ...
        entry: Dict[str, Any] = {
            ...
            # stop_reason: structured termination classification (Hermes P1 improvement)
            # Maps exit_reason + status to canonical categories for replay evaluation
            # conversation_loop _turn_exit_reason values are fine-grained; collapse
            # them into the canonical replay taxonomy so cross-version drift is
            # measurable (goal_achieved vs budget_exhausted vs timeout vs tool_error
            # vs context_overflow vs interrupted).
            "stop_reason": _classify_stop_reason(exit_reason, status),
            "tokens": {
                "input": (
