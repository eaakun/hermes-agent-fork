...
class _SSRFGuardedAsyncNetworkBackend:
    ...
    async def connect_tcp(
        ...
        for ip in ips:
            # Re-validate each resolved IP directly before dialing. The
            # getaddrinfo result can change between the time _resolved_http_connect_ips
            # validated it and now (DNS-rebinding / TOCTOU), so never hand a raw IP
            # to the underlying backend without a final blocked-list check. If the
            # IP is dangerous we refuse to connect and surface SSRFConnectionBlocked
            # instead of leaking a connection attempt to a metadata/private host.
            try:
                raw_ip = ipaddress.ip_address(ip)
            except ValueError:
                raise SSRFConnectionBlocked(
                    f"Blocked request - unparseable IP address {ip!r} for hostname {host}"
                )
            if raw_ip in _ALWAYS_BLOCKED_IPS or any(
                raw_ip in net for net in _ALWAYS_BLOCKED_NETWORKS
            ):
                raise SSRFConnectionBlocked(
                    f"Blocked request to cloud metadata address during connect: {host} -> {ip}"
                )
            if _is_blocked_ip(raw_ip):
                raise SSRFConnectionBlocked(
                    f"Blocked request to private/internal address during connect: {host} -> {ip}"
                )
            try:
        ...
        if last_exc is not None:
            # All resolved IPs failed to connect. Surface SSRFConnectionBlocked
            # (not the raw transport error) so callers get a consistent,
            # security-flavored exception and so httpx's httpcore->httpx
            # exception mapping can't mask it as a generic ConnectError.
            raise SSRFConnectionBlocked(
                f"Blocked request - all connect attempts to {host}:{port} failed: {last_exc}"
            )
...
class _SSRFGuardedNetworkBackend:
    ...
    def connect_tcp(
        ...
        last_exc: Exception | None = None
        for ip in ips:
            # Re-validate each resolved IP directly before dialing (see async
            # backend for the rationale — TOCTOU / DNS-rebinding defense).
            try:
                raw_ip = ipaddress.ip_address(ip)
            except ValueError:
                raise SSRFConnectionBlocked(
...
def create_ssrf_safe_async_client(**kwargs: Any) -> Any:
    ...
    ``trust_env`` is forced ``False`` unless the caller explicitly passes it:
    honoring system/environment proxy settings would route the request through
    a proxy, which resolves and dials the *real* target on our behalf — silently
    bypassing the connect-time IP allow-list below.  A "trusted egress proxy"
    cannot coexist with per-connection SSRF validation, so we never inherit one.
    """
    import httpx
...
def create_ssrf_safe_client(**kwargs: Any) -> Any:
    ...
    ``trust_env`` is forced ``False`` (see ``create_ssrf_safe_async_client``)
    so a system/environment proxy cannot bypass the connect-time IP validation.
    """
    import httpx
