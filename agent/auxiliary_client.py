...
def resolve_provider_client(
    ...
    # OpenCode Zen keyless free tier (opencode-free): served anonymously on the
    # Zen relay with no API key. Resolve it through the backported keyless
    # runtime so the centralized fallback router (and thus try_activate_fallback)
    # can rotate to a live free model without any credential. The SDK gets a
    # non-empty placeholder api_key (so it emits a header) plus an empty
    # Authorization default header — the wire form the relay accepts for the
    # anonymous free tier. Mirrors resolve_runtime_provider()'s early return.
    if provider == "opencode-free":
        try:
            from hermes_cli.models import opencode_zen_free_runtime
            _fr = opencode_zen_free_runtime(
                "opencode-free", model or "laguna-s-2.1-free"
            )
            if _fr is not None:
                from openai import OpenAI, AsyncOpenAI
                _fb_factory = AsyncOpenAI if async_mode else OpenAI
                _fb_client = _fb_factory(
                    api_key=_fr["api_key"],
                    base_url=_fr["base_url"],
                    default_headers=_fr.get("default_headers") or {},
                    max_retries=0,
                )
                return (
                    _fb_client,
                    _fr.get("model") or model or "laguna-s-2.1-free",
                )
        except Exception as _oc_err:
            logger.debug("opencode-free client resolution failed: %s", _oc_err)
        return (None, None)
...
def _resolve_strict_vision_backend(
    ...
    if provider == "huggingface":
        # [2026-09-08] HuggingFace Inference Router (OpenAI-compatible,
        # https://router.huggingface.co/v1) — supports vision chat models
        # like Qwen2.5-VL-7B-Instruct. Free tier: 100K credits/month.
        # _get_aux_model_for_provider("huggingface") has no entry, so we
        # explicitly read providers.huggingface.model from config.yaml.
        if not model:
            try:
                from hermes_cli.config import load_config, cfg_get
                _cfg = load_config()
                model = cfg_get(_cfg, "providers", "huggingface", "model", default="")
            except Exception:
                model = ""
            if not model:
                logger.debug(
                    "Vision auto-detect: huggingface requires explicit model "
                    "(set providers.huggingface.model in config.yaml)"
                )
                return None, None
        return resolve_provider_client("huggingface", model, is_vision=True)
    if provider == "custom":
...
# is kept unchanged.
#
# Hardened 2026-08-28: lowered from 300s to 60s so a slow/unstable upstream
# proxy (e.g. the warp tunnel at 127.0.0.1:18086 that all LLM calls route
# through) cannot make a single compression pass hang for 5 minutes and pin a
# gateway turn in "API error recovery" until it idles out (1800-3600s).  The
# compressor already falls back to the deterministic context marker on timeout,
# so a bounded deadline is strictly safer — fast on a healthy link, fast-fail
# on a sick one.  The configured ``auxiliary.compression.timeout`` still wins
# when set above this floor.
