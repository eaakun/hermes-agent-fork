...
# Cooldown applied when leaving the primary due to billing / subscription
# quota exhaustion (e.g. MiniMax Token Plan). This is longer than the 60s
# rate-limit cooldown because a quota exhaustion typically resolves on a
# schedule (monthly billing cycle) rather than within seconds.  The watchdog
# independently probes MiniMax every 30 min and will restart the gateway to
# clear the fallback state when recovery is detected.
_BILLING_FALLBACK_COOLDOWN_S = 300  # 5 minutes
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    mappings.
    """
    # Free-pool health feedback: this switch only fires after the current model
    # has failed its retries, so demote it in the rotation ranking. Guarded so a
    # failure here can never interfere with the fallback itself.
    try:
        _fm = str(getattr(agent, "model", "") or "").strip()
        if "-free" in _fm:
            from hermes_cli.models import record_free_model_outcome
            record_free_model_outcome(_fm, False)
    except Exception:
        pass
    ...
    if reason in {FailoverReason.rate_limit, FailoverReason.billing, FailoverReason.upstream_rate_limit}:
        ...
        if (not fallback_already_active) or (primary_provider and current_provider == primary_provider):
            if reason == FailoverReason.billing:
                agent._rate_limited_until = time.monotonic() + _BILLING_FALLBACK_COOLDOWN_S
            else:
