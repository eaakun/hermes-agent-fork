...
def _classify_by_status(
    ...
    *,
    ...
    if status_code == 429:
        ...
        # Check billing / quota exhaustion (e.g. MiniMax "Token Plan 用量上限").
        # These indicate a subscription-level quota, not a transient rate limit,
        # so the recovery path should use a longer cooldown and avoid hammering
        # an exhausted bucket.
        if any(p in error_msg for p in _BILLING_PATTERNS):
            return result_fn(
                FailoverReason.billing,
